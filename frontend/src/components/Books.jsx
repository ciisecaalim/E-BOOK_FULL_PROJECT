...
import { 
  FaCheckCircle, FaExclamationTriangle, FaTimesCircle, FaTrash, 
  FaFileCsv, FaSort, FaSortUp, FaSortDown 
} from "react-icons/fa";
...
function BookTable() {
  ...
  useEffect(() => {
    books.forEach((book) => {
      if (book.quantity === 0) toast.error(`❌ "${book.name}" is Out of Stock!`);
      else if (book.quantity < 5) toast.warn(`⚠️ "${book.name}" is Low Stock (${book.quantity} left)`);
    });
  }, [books]);
    ...
    <div className="space-y-8 relative">
      ...
      <div className="flex justify-between items-center p-6 bg-blue-700 text-white rounded-2xl shadow">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={exportCSV} className="px-3 py-2 bg-green-500 rounded-lg flex items-center gap-2"><FaFileCsv /> CSV</button>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow">
        ...
        <div className="flex items-center gap-2">
          ...
          <button disabled={page>=totalPages-1} onClick={()=>setPage(p=>p+1)} className={`px-3 py-1 rounded ${page>=totalPages-1?"bg-gray-300":"bg-blue-600 text-white"}`}>Next</button>
        </div>
      </div>

      {/* ADD BOOK BUTTON */}
      <Link to="/add/book" className="fixed bottom-10 right-10 bg-blue-700 text-white rounded-full w-16 h-16 flex items-center justify-center text-3xl hover:scale-110">+</Link>

      {/* IMAGE VIEW */}
      {viewImage && <div onClick={()=>setViewImage(null)} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50"><img src={viewImage} className="w-96 rounded-xl shadow-lg border-4 border-white"/></div>}
      ...
      {confirmDelete && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center w-80">
          ...
          <div className="flex justify-center gap-4 mt-4">
            <button onClick={()=>setConfirmDelete(null)} className="px-5 py-2 rounded border">Cancel</button>
            <button onClick={()=>deleteProduct(confirmDelete)} className="px-5 py-2 bg-red-600 text-white rounded">Delete</button>
          </div>
        </div>
      </div>}
