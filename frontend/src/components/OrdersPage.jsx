...
function OrdersPage() {
  ...
  // --- First user interaction for autoplay audio ---
  useEffect(() => {
  ...
  // --- Alerts: Low stock & unpaid orders ---
  useEffect(() => {
    ...
    if (unpaidOrders.length > prevUnpaidCount) {
      const newUnpaid = unpaidOrders.length - prevUnpaidCount;
      toast.info(`💳 ${newUnpaid} new unpaid order${newUnpaid > 1 ? "s" : ""}!`);
      beepAudio.current?.play().catch(() => console.log("Audio play blocked"));
    }
    setPrevUnpaidCount(unpaidOrders.length);
  }, [orders, products, userInteracted, prevUnpaidCount]);
  ...
  }, [orders, searchKey, statusFilter, dateFilter]);
  ...
  const exportCSV = () => {
    ...
    const rows = data.map(o => [
      escapeCSV(o.customer || "Guest"),
      escapeCSV(o.email || "N/A"),
      escapeCSV(o.status || "Processing"),
      escapeCSV(o.paymentStatus || "Unpaid"),
      escapeCSV(new Date(o.createdAt).toLocaleString()),
      escapeCSV(o.products.map(p => `${p.productId?.name || p.name || "Unknown"} (x${p.quantity})`).join("; ")),
  ...
  const printOrders = () => {
    ...
    const html = `<html><head><title>Orders</title><style>
      table{width:100%;border-collapse:collapse;}
      th,td{border:1px solid #333;padding:8px;}
      th{background:#f0f0f0;position:sticky;top:0;}
      </style></head><body><h2>Orders Report</h2><table><thead><tr><th>#</th><th>Customer</th><th>Status</th><th>Payment</th><th>Date</th><th>Products</th><th>Total</th></tr></thead><tbody>${
        data.map((o,i) => `<tr>
        <td>${i+1}</td>
        ...
        <td>${o.products.map(p => {
          const prod = products.find(pr => pr._id === (p.productId?._id || p._id));
          const low = prod?.quantity <= (prod?.minQuantity || 5);
          return `<span style="${low ? 'color:#ca8a04;font-weight:bold' : ''}">${p.productId?.name || p.name || "Unknown"} (x${p.quantity})</span>`;
        }).join("; ")}</td>
  ...
  return (
    <div className="space-y-8 p-6 bg-gray-50 min-h-screen">
      ...
      {/* Alert Panel */}
      {(products.some(p => p.quantity <= (p.minQuantity || 5)) || orders.some(o => o.paymentStatus !== "Paid")) && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-xl shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div className="text-yellow-700 text-sm font-medium">
            ⚠️ Low Stock: {products.filter(p => p.quantity <= (p.minQuantity || 5)).map(p => `${p.name} (${p.quantity})`).join(", ")}
          </div>
          <div className="text-blue-700 text-sm font-medium">
            💳 {orders.filter(o => o.paymentStatus !== "Paid").length} unpaid order(s)
          </div>
        </div>
      )}
      ...
      {/* Table */}
      {loading ? (
        <div className="flex justify-center py-20 text-gray-500"><FaSpinner className="animate-spin mr-2" /> Loading...</div>
      ) : error ? (
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {currentOrders.map((o,i) => {
                const hasLowStock = o.products.some(p => {
                  const prod = products.find(pr => pr._id === (p.productId?._id || p._id));
                  return prod?.quantity <= (prod?.minQuantity || 5);
                });
                return (
                  <React.Fragment key={o._id}>
                    <tr className={`${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'} ${o.paymentStatus !== "Paid" ? "bg-red-50" : ""} ${hasLowStock ? "border-l-4 border-yellow-400" : ""} hover:bg-gray-100`}>
                        ...
                        <td colSpan="8" className="p-4">
                          <div className="space-y-3">
                            ...
                            <div className="bg-white p-3 rounded-xl shadow-sm border">
                              ...
                              {o.products.map((p,j) => {
                                const prod = products.find(pr => pr._id === (p.productId?._id || p._id));
                                const isLowStock = prod?.quantity <= (prod?.minQuantity || 5);
                                return (
                                  <div key={j} className="flex justify-between items-center text-sm mb-1">
                                    <span className={isLowStock ? "font-bold text-yellow-600" : ""}>{p.productId?.name || p.name || "Unknown"}</span>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setPage(prev => Math.max(prev - 1, 0))} disabled={page===0} className="px-3 py-1 bg-gray-300 rounded-lg disabled:opacity-50">Prev</button>
          <button onClick={() => setPage(prev => Math.min(prev + 1, totalPages - 1))} disabled={page===totalPages-1} className="px-3 py-1 bg-gray-300 rounded-lg disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
  );
